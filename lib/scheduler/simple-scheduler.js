...
(function() {
	...
	this.acquireWorker = function(appSpec, url){
		var freeWorkers = _.filter(_.values(this.$workers), function(w) { return w.data.pendingConn === 0 });

	    if (this.$workers && (_.size(this.$workers) >= appSpec.settings.scheduler.simple.maxWorkers ||
			_.size(freeWorkers) > 0)) {
	      logger.trace('Reusing existing instance');

	      var thisWorker = _.sortBy(this.$workers, function(w) { return w.sessionCount() - w.data.pendingConn })[0];
